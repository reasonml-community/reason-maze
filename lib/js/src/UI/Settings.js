...
function maybe_get(ar, count, n) {
  var match = +(n < count);
  if (match !== 0) {
    return /* Some */[Caml_array.caml_array_get(ar, n)];
  } else {
    return /* None */0;
  }
}
...
function from_json(str) {
  ...
  if (parsed) {
    ...
    if (match) {
      ...
      var board = $great$unknown($great$great$eq$eq($great$great$eq$eq(maybe_get(items, count, 0), Js_json.decodeString), Types.Board[/* by_name */3]), /* Hexagon */2);
      var algorithm = $great$unknown($great$great$eq$eq($great$great$eq$eq(maybe_get(items, count, 1), Js_json.decodeString), Types.Alg[/* by_name */3]), /* DFS */0);
      var fill = $great$unknown($great$great$eq$eq(maybe_get(items, count, 2), parse_fill), /* HueSat */[/* tuple */[
              30,
              90
            ]]);
      var wall = $great$great$eq$eq(maybe_get(items, count, 3), parse_line);
      var edge = $great$great$eq$eq(maybe_get(items, count, 4), parse_line);
      var batch_size = $great$unknown($great$great$eq($great$great$eq$eq(maybe_get(items, count, 5), Js_json.decodeNumber), (function (prim) {
                  return prim | 0;
                })), 1);
      var size_hint = $great$unknown($great$great$eq($great$great$eq$eq(maybe_get(items, count, 6), Js_json.decodeNumber), (function (prim) {
                  return prim | 0;
                })), 10);
      var canvas_size = $great$unknown($great$great$eq($great$great$eq$eq(maybe_get(items, count, 7), Js_json.decodeNumber), (function (prim) {
                  return prim | 0;
                })), 1000);
      var seed = $great$unknown($great$great$eq($great$great$eq$eq(maybe_get(items, count, 8), Js_json.decodeNumber), (function (prim) {
                  return prim | 0;
                })), 0);
      return /* Some */[/* record */[
                /* board */board,
