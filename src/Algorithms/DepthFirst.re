...
module F (Config: Config) => {
  module State = {
    ...
    let current t => List.map (fun {Shared.Edge.dest: dest} => dest) (Array.to_list t.current);
    let next t =>
      switch t.active {
  ...
  let get_new state => {
    ...
    if (Array.length current === 0) {
      ...
      switch (
        Js.Array.spliceInPlace pos::(Random.int (Array.length current)) remove::1 add::[||] current
      ) {
  ...
  let step state => {
    ...
    | Some (src, hits) =>
      if visited.(src.dest) {
        ...
        let edges = List.filter (fun x => x.Shared.Edge.dest !== src.src) edges;
        let age = age + 1;
