...
module F (Board: SimpleBoard.T) (Gen: Generator.T) => {
  ...
  let create_enabled_map coords => {
    Array.fold_left
      (fun map c => CoordMap.add c false map)
      CoordMap.empty
      coords
  };
  ...
  let toggle_all state coord => {
    open State;
    let enabled = Board.adjacents state.shape coord
  ...
  let paint_shapes {State.coords, scale, shape, enabled} => {
    ...
    /*List.map*/
    List.filter (fun coord => CoordMap.find coord enabled)
