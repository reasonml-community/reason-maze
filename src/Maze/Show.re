...
module F (Board: SimpleBoard.T) (Gen: Generator.T) => {
  ...
  let show_debug ctx options state => {
    ...
    Generator.PairSet.iter (fun (a, b) => Js.log (a, b)) edges;
    let walls = Man.all_walls state;
    ...
    | Some get_color =>
      ...
      Array.iteri
        (Presenter.draw_shapei ctx (xm, ym) get_color (Man.current_age state) (Man.max_age state))
        (Man.all_shapes state)
    | None => ()
    };
    switch options.draw_edges {
  ...
  let animate ctx batch_size interval options state onStop => {
    ...
    let rec inner state => {
      ...
      Man.finished state ? onStop () : id := Window.setTimeout (fun () => inner state) interval
    };
    inner state;
    id
  };
