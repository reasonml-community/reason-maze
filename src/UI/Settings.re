...
module T = {
  ...
  let parse_fill = fill =>
    Js.Json.(
      ...
      | JSONArray([|hue, sat|]) =>
        switch (classify(hue), classify(sat)) {
        | (JSONNumber(hue), JSONNumber(sat)) =>
          Some(HueSat((int_of_float(hue), int_of_float(sat))))
        | _ => None
        }
      | _ => None
      }
...
let to_jsonable = t =>
  T.(
    ...
    T.fill_json(t.fill),
    Js.Null.fromOption(t.wall),
    Js.Null.fromOption(t.edge),
    t.batch_size,
    t.size_hint,
    t.canvas_size,
    t.seed,
...
let parse_line = value =>
  Js.Json.(
    switch (classify(value)) {
    | JSONArray([|width, light|]) =>
      switch (classify(width), classify(light)) {
      | (JSONNumber(width), JSONNumber(light)) =>
        Some((int_of_float(width), int_of_float(light)))
      | _ => None
      }
    | _ => None
    }
...
let from_json = str => {
  ...
  | Some(arr) =>
    ...
    | Some(items) =>
      ...
      let algorithm =
        item(1) >>== decodeString >>== Types.Alg.by_name >? initial.algorithm;
      let fill = item(2) >>== T.parse_fill >? initial.fill;
      let wall = item(3) >>== parse_line;
      let edge = item(4) >>== parse_line;
      let batch_size =
        item(5) >>== decodeNumber >>= int_of_float >? initial.batch_size;
      let size_hint =
        item(6) >>== decodeNumber >>= int_of_float >? initial.size_hint;
      let canvas_size =
        item(7) >>== decodeNumber >>= int_of_float >? initial.canvas_size;
...
let to_options = (settings: T.t) =>
  Show.Options.{
    canvas_size: (
      float_of_int(settings.canvas_size),
      float_of_int(settings.canvas_size),
    ),
    min_margin: 10.0,
    ...
    draw_edges:
      ...
      | Some((width, lightness)) =>
        Some((float_of_int(width), Presenter.hsl(0, 0, lightness)))
      | None => None
      },
    draw_shapes:
        ...
        Some(
          (
            (current, _max) =>
              Presenter.hslf(
                float_of_int(hue),
                float_of_int(saturation),
                100.0 -. current *. 50.0,
              )
          ),
        )
      | Rainbow =>
        Some(((_current, max) => Presenter.hslf(max *. 360.0, 70.0, 50.0)))
      | NoFill => None
      },
    draw_walls:
      switch (settings.wall) {
      | Some((width, lightness)) =>
        Some((float_of_int(width), Presenter.hsl(0, 0, lightness)))
      | None => None
      },
