...
let render = (width, height, canvas) => {
  ...
  for (x in 0 to width) {
    for (y in 0 to height) {
      let color =
        Presenter.hsl(scale(x, width, 360), scale(y, height, 100), 50);
...
let setCanvasRef = (r, {ReasonReact.state}) =>
  state.canvas := Js.Nullable.toOption(r);
    ...
    <div style=(ReactDOMRe.Style.make(~position="relative", ()))>
      ...
      (
        ...
        | Some((hue, saturation)) =>
          ...
          let style =
            ReactDOMRe.Style.unsafeAddProp(style, "pointerEvents", "none");
          <div style />;
        }
      )
    </div>,
